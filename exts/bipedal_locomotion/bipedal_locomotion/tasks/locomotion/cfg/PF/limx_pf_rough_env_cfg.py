...
from .terrains_cfg import BLIND_ROUGH_TERRAINS_CFG
...
@configclass
class PFRoughSceneCfg(InteractiveSceneCfg):
    """Configuration for the rough scene"""

    # 地形配置 / Terrain configuration
    terrain = TerrainImporterCfg(
        prim_path="/World/ground",      # 地形在场景中的路径 / Terrain path in scene
        terrain_type="generator",           # 地形类型：生成器 / Terrain type: generator
        terrain_generator=BLIND_ROUGH_TERRAINS_CFG,         # 使用盲视粗糙地形生成器 / Use blind rough terrain generator
    ...
    left_foot_height_scanner = RayCasterCfg(
        prim_path="{ENV_REGEX_NS}/Robot/foot_L_Link",
        offset=RayCasterCfg.OffsetCfg(pos=(0.0, 0.0, 20.0)),
        attach_yaw_only=True,
        pattern_cfg=patterns.GridPatternCfg(resolution=0.01, size=(0.06, 0.06)),
        debug_vis=False,
        mesh_prim_paths=["/World/ground"],
    )

    right_foot_height_scanner = RayCasterCfg(
        prim_path="{ENV_REGEX_NS}/Robot/foot_R_Link",
        offset=RayCasterCfg.OffsetCfg(pos=(0.0, 0.0, 20.0)),
        attach_yaw_only=True,
        pattern_cfg=patterns.GridPatternCfg(resolution=0.01, size=(0.06, 0.06)),
        debug_vis=False,
    ...
    contact_forces = ContactSensorCfg(
        ...
        history_length=3,                     # 历史数据长度 / History data length
...
class CommandCfg(BaseCommandsCfg):
    ...
    def __post_init__(self):
        ...
        self.base_velocity.rel_heading_envs = 1.0        # 航向环境比例 / Heading environments ratio
...
class ObservarionsCfg:
    ...
    class PolicyCfg(ObsGroup):
        ...
        base_ang_vel = ObsTerm(
            ...
            noise=Unoise(n_min=-0.2, n_max=0.2),  # Unoise
        ...
        proj_gravity = ObsTerm(
            ...
            noise=Unoise(n_min=-0.05, n_max=0.05),  # 噪声配置 / Noise configuration
            clip=(-100.0, 100.0),               # 裁剪范围 / Clipping range
        ...
        joint_pos = ObsTerm(
            ...
            noise=Unoise(n_min=-0.01, n_max=0.01),  # 噪声配置 / Noise configuration
            clip=(-100.0, 100.0),               # 裁剪范围 / Clipping range
            scale=1.0,                          # 缩放因子 / Scaling factor
        )
        joint_vel = ObsTerm(
            func=mdp.joint_vel,                 # 关节速度函数 / Joint velocity function
            noise=Unoise(n_min=-1.5, n_max=1.5),  # 噪声配置 / Noise configuration
            clip=(-100.0, 100.0),               # 裁剪范围 / Clipping range
    ...
    class HistoryObsCfg(ObsGroup):
        ...
        proj_gravity = ObsTerm(func=mdp.projected_gravity, noise=Unoise(n_min=-0.05, n_max=0.05),clip=(-100.0, 100.0),scale=1.0,)

        # robot joint measurements
        joint_pos = ObsTerm(func=mdp.joint_pos_rel, noise=Unoise(n_min=-0.01, n_max=0.01),clip=(-100.0, 100.0),scale=1.0,)
        joint_vel = ObsTerm(func=mdp.joint_vel, noise=Unoise(n_min=-1.5, n_max=1.5),clip=(-100.0, 100.0),scale=0.25,)
    ...
    class CriticCfg(ObsGroup):
        ...
        left_foot_clearance = ObsTerm(
            func=mdp.foot_clearance_flag,
            params={
                "sensor_cfg": SceneEntityCfg("left_foot_height_scanner"),
                "contact_sensor_cfg": SceneEntityCfg("contact_forces", body_names="foot_.*"),
                "foot_radius": 0.032,
                "contact_threshold": 1.0,
                "clearance_threshold": 0.05,
            },
        )
        right_foot_clearance = ObsTerm(
            func=mdp.foot_clearance_flag,
            params={
        ...
        robot_feet_lin_vel = ObsTerm(
            func=mdp.feet_lin_vel,
            params={"asset_cfg": SceneEntityCfg("robot", body_names="foot_.*")},
        )
...
class RewardsCfg:
    ...
    pen_joint_torque = RewTerm(func=mdp.joint_torques_l2, weight=-2e-5)
    pen_joint_accel = RewTerm(func=mdp.joint_acc_l2, weight=-2.5e-07)
    pen_action_rate = RewTerm(func=mdp.action_rate_l2, weight=-0.03)
    pen_joint_pos_limits = RewTerm(func=mdp.joint_pos_limits, weight=-10.0)
    pen_joint_vel_l2 = RewTerm(func=mdp.joint_vel_l2, weight=-5e-05)
    
    no_fly = RewTerm(
        func=mdp.no_fly,
        weight=1.0,
        params={"sensor_cfg": SceneEntityCfg("contact_forces", body_names="foot_.*"), "threshold": 5.0},
    )
    ...
    feet_air_time = RewTerm(
        ...
        params={
            ...
            "min_feet_air_time": 0.17,
            "max_feet_air_time": 0.3,
        },
    )
    feet_air_time_sync = RewTerm(
        func=mdp.feet_air_time_sync,
        weight=2.0,
        params={
            "sensor_cfg": SceneEntityCfg("contact_forces", body_names="foot_.*"),
            "sync_scale": 0.05,
    ...
    max_feet_height_sync = RewTerm(
        func=mdp.max_feet_height_sync,
        weight=1.0,
        params={
            "asset_cfg": SceneEntityCfg("robot", body_names=["foot_L_Link", "foot_R_Link"]),
            "left_scanner_cfg": SceneEntityCfg("left_foot_height_scanner"),
            "right_scanner_cfg": SceneEntityCfg("right_foot_height_scanner"),
            "height_sigma": 0.05,
        },
    )
...
class TerminationsCfg:
    ...
    bad_orientation = DoneTerm(
        ...
        params={
            "roll_limit_angle": 1.3,
            "pitch_limit_angle": 1.6,
            "probability": 0.02,
...
@configclass
class PFRoughEnvCfg(ManagerBasedRLEnvCfg):
    """测试环境配置类 / Test environment configuration class"""

    # 场景设置 / Scene settings
    scene: PFRoughSceneCfg = PFRoughSceneCfg(num_envs=8192, env_spacing=2.5)
    # 基本设置 / Basic settings
    ...
    def __post_init__(self):
        ...
        self.sim.render_interval = self.decimation  # 渲染间隔 / Rendering interval
